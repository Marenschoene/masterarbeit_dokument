...
Die von \textcite[S. 1]{semanticMatchmaking:2009} implementierte Recommener Engine wird auch als hybrides Empfehlungssystem bezeichnet. Solche Systeme kombinieren mehrere Vorschlagsverfahren innerhalb einer Anwendung \cite[S. 8]{malinowski:2008}.

\section{Hybride Empfehlungssysteme}
\label{ch:empfehlungssysteme:hybrideEmpfehlungssysteme}
Durch die gleichzeitige Verwendung unterschiedlicher Empfehlungsansätze innerhalb einer Anwendung kombinieren hybride Recommender Engines die Vorteile mehrerer der bisher vorgestellten Verfahren innerhalb eines Systems \cite[S. 199f.]{recommenderSystems:2016}.

Beispielsweise entwickelten \textcite{shalaby:2017} ein hybrides Empfehlungssystem zur Stellensuche, in welchem sie Methoden des kollaborativen und inhaltsbasierten Filtern miteinander kombinierten.
...
\textcite[S. 1]{malinowski:2008} kritisieren jedoch, dass auch ein hybrides Empfehlungssystem nicht ausreicht, um geeignete Mitarbeiter für Stellen zu empfehlen. Deren Einschätzung zu Folge ist die Entwicklung bilateraler Empfehlungssysteme notwendig.
