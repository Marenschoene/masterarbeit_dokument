...
Beispiele für Attribute eines Nutzers sind das Geschlecht oder Alter eines Anwenders, während Attribute eines Produktes beispielsweise die Produktkategorie oder beschreibende Schlagwörter darstellen.
...
\begin{equation}\label{eq1}
    s'_c = arg\max_{s \in S} R(c,s)
\end{equation}
Hierbei bezeichnet $C$ die Menge aller Nutzer und $S$ die Menge aller Elemente eines Systems.
$R(c,s)$ gibt den Nutzen einer Kombination $(c,s)$ von Element $s$ und Nutzer $c$ (im Folgenden \ac{N-E-K} genannt \cite[S. 3]{recommenderSystems:2016}) an \cite[S. 734f.]{adomavicius:inproceedings}\cite[S. 219]{lakiotaki:inproceedings}.
...
\end{equation}
Die Nutzenfunktion ordnet jeder Kombination $(c_{i},s_{j})$ von Nutzer $c_{i}$ und Element $s_{j}$ eines Empfehlungssystems einen Nutzen $R(c_{i},s_{j}) \in R_{0}$ zu.
$R_{0}$ umfasst alle Werte, die der Nutzen von Kombinationen annehmen kann \cite[S. 49f]{adomavicius:inproceedings:2}.

Die Nutzenfunktion $R$ kann auch als Matrix $R[.,.]$ abgebildet werden \cite[S. 1]{dekhtyar:misc}.
Jeder Eintrag $r_{c_{i}s_{j}}$ der Matrix stellt den Nutzen $R(c_{i},s_{j})$ eines Elements $s_{j}$ für einen Nutzer $c_{i}$ dar (d.h. $r_{c_{i}s_{j}} = R(c_{i},s_{j})$) \cite[S. 1]{dekhtyar:misc}.
...
Besteht das Ziel eines Empfehlungssystems bespielsweise darin, den Gesamtumsatz eines Unternehmens zu maximieren, so kann der Nutzen einer \ac{N-E-K} über eine Profit-Funktion ermittelt werden \cite[S. 735]{adomavicius:inproceedings}\cite[S. 11]{recommenderSystems:2016}\cite[S. 1]{jannach:article}.
...
In den meisten Empfehlungssystemen wird für den Nutzen $R(c,s)$ unmittelbar die Bewertung (engl.: Rating) eines Element $s$ durch einen Nutzer $c$ angenommen \cite[S. 735]{adomavicius:inproceedings}\cite[S. 9]{ricci:inbook}\cite[S. 11]{recommenderSystems:2016}.
...
Aus Gründen der Einfachheit wird für den weiteren Verlauf der Arbeit ebenfalls der Wert einer Bewertung für den Nutzen $R(c,s)$ einer \ac{N-E-K} $(c,s)$ angenommen.
% Die Begrifflichkeiten Nutzenfunktion, Ratingfunktion, Ratingmatrix und Ratingfunktion werden daher nachfolgend synonym verwendet.
...
Ziel ist es, in Abhängigkeit der vollständigen Rating-Matrix $R$ eine Empfehlung an einen Nutzer $c$ zu generieren, die das bzw. die Elemente $s'$ mit dem größtmöglichen Nutzen enthält \cite[S. 87]{ekstrand:article}\cite[S. 898]{adomavicius:article}.
...
In den meisten Fällen geben Empfehlungssysteme eine sortierte Liste der nützlichsten Elemente zurück (Top-K-Elemente), in der für jedes Element der angenommene Wert für einen Nutzer angegeben ist \cite[S. 6]{ricci:inbook}\cite[S. 3]{recommenderSystems:2016}\cite[S. 141]{ekstrand:article}.
Der Rang eines Elements in der sortierten Liste ergibt sich anhand des Sortier-Kriteriums Nutzen.
...
In den meisten Empfehlungssystemen wird demnach der Rang eines Elements über das Sortier-Kriterium Rating ermittelt (bspw. der Gesamtbewertung eines Elements durch einen Nutzer).
Formal ausgedrückt steht ein Element $s_{x}$ höher in der sortierten Liste als ein Element $s_{y}$, wenn $rank(s_{x})<rank(s_{y})$, wobei $rank: S \rightarrow \mathbb{R}$.
Über die Funktion $rank$ wird jedem Element $s \in S$ basierend auf dem Ranking-Kriterium ein Rang zugeordnet.
Für das Ranking existieren verschiedene Verfahren.
Ein bekanntes Verfahren stellt die Standard-Ranking-Methode dar, wobei der Rang eines Elements über das Inverse der (angenommenen) Gesamtbewertung eines Nutzers $c$ für das Element $s$ bestimmt werden kann.
...
In der Praxis kann es vorkommen, dass von einem (vorhergesagten) Wert einer \ac{N-E-K} nicht unmittelbar auf den tatsächlichen Rang einer Kombination im Ranking geschlossen werden kann.
