...
Die Angestellten des Unternehmens erhalten Übersichten über offenen Projektpositionen. Daraufhin bewerten sie auf einer vordefinierten Skala, wie zufrieden sie mit einer Tätigkeit auf den vorliegenden Projektpositionen wären. Hierbei wird für beide Empfehlungssysteme überprüft, ob eine hohe erwartete Zufriedenheit der Angestellten mit einer hohen Positionierung in den Ergebnislisten korreliert.
...
Die Mitarbeiter der EXXETA AG und deren Fähigkeiten werden in Form eines Graphen dargestellt. Um das Sparsity Problem zu beheben, wird das Verfahren von Katz angewendet. Dieser speicherbasierte Graphenalgorithmus wurde in Kapitel \ref{ch:empfehlungssysteme:cf:speicherbasiert} vorgestellt. Vorteilhaft gegenüber modellbasierten Methoden ist insbesondere die Langlebigkeit des Ansatzes. Sollten sich nach Durchführung des Experiments Daten im Unternehmen signifikant verändern oder neue Kompetenzen im Intranet hinzugefügt werden, ist das speicherbasierte Verfahren weiterhin ohne zusätzlichen manuellen Aufwand im Betrieb anwendbar. Die zu erwartende hohe Komplexität ist in der vorliegenden Problemstellung tolerierbar, da sich zum Zeitpunkt des Experiments unter 1.000 Mitarbeiter im Unternehmen befinden.
...
Wie in Kapitel \ref{ch:empfehlungssysteme:cf:speicherbasiert} beschrieben, ändern sich bei Berechnung des Algorithmus von Katz die Kompetenzbewertungen. Somit es ist nicht ohne Weiteres möglich, die Eingaben eines Projektmanagers mit den Werten in der Ergebnis-Matrix zu vergleichen.

Jedoch ist festzustellen, dass sich die Beurteilungen, welche ursprünglich einem gemeinsamen Kompetenzniveau entsprachen, auch nach Berechnung des Katz-Algorithmus auf einem vergleichbaren Niveau befinden. Die Werte innerhalb eines Kompetenzbereichs sind lediglich feingranular unterschiedlich. Dieses Phänomen ist in Tabelle \ref{tbl:methodik:versuchsaufbau:unilateral:tbl1} zu beobachten. Dort sind die Bewertungen der Fähigkeit Python aus Abbildung \ref{fig:methodik:versuchsaufbau:unilateral:abb1} vor und nach der Berechnung des Katz-Algorithmus eingetragen. Gleiche Kompetenzniveaus sind in der Tabelle durch einheitliche Hintergrundfarben gekennzeichnet.
...
\begin{table}[h]
	...
	\begin{tabular}{c|c|c|c}
		...
		\hline
		\rowcolor{exxetagray} Keine Kenntnisse & Erika M.  & 0                  & 0.43\\
		\rowcolor{exxetagray} Keine Kenntnisse & John D.   & 0                  & 0.92\\
		\hline
		\rowcolor{usercolor}  Grundkenntnisse  & Max M.    & \kantengewicht     & 1.93\\
		\hline
		\rowcolor{itemcolor}  Fortgeschritten  & Jane D.   & \kantengewichtHoch & 2.77
	\end{tabular}
	\caption{Ergebnisse des Katz-Algorithmus für die Kompetenz Python im Graphen aus Abbildung \ref{fig:methodik:versuchsaufbau:unilateral:abb1}}
	\label{tbl:methodik:versuchsaufbau:unilateral:tbl1}
\end{table}

Würde ein Projektmanager einen Mitarbeiter mit einem bestimmten Kenntnisniveau in der Programmiersprache Python suchen, können die Daten in Tabelle \ref{tbl:methodik:versuchsaufbau:unilateral:tbl1} dennoch nach wie vor verwendet werden. Hierfür wird der Angestellte mit dem höchstem höchsten Ergebnis des Katz-Algorithmus im gesuchten Kompetenzbereich als geeignetster Mitarbeiter für die vorliegenden Fähigkeit definiert. Zur Bestimmung des \acp{PEFit} dient dessen Bewertung als Nullpunkt in Abbildung \ref{fig:methodik:versuchsaufbau:unilateral:abb2}, welche bereits in Kapitel \ref{ch:personEnvironmentFit:auswirkungenErhoehterAngebote} diskutiert wurde.
...
Zur Bestimmung des Anforderungen-Fähigkeiten Fits wird angenommen, dass Projektmanager anstreben, offene Projektpositionen mit fachlich optimal passenden Mitarbeitern zu besetzen. Folglich vermeiden sie sowohl eine Über- als auch eine Unterqualifizierung ihrer Angestellten. Aus diesem Grund wird Kurve B aus Abbildung \ref{fig:methodik:versuchsaufbau:unilateral:abb2} in der unilateralen Empfehlungskomponente durch quadrierte Differenzberechnung implementiert.

Der Algorithmus der unilateralen Empfehlungskomponente wird anhand der folgenden beispielhaften Projektposition erläutert.
