...
Die Fähigkeitsbewertungen und Teamzuordnungen können für alle Mitarbeiter der EXXETA AG über eine \acsu{REST}-Schnittstelle in Form von \acsu{JSON} abgefragt werden. Quellcode \ref{qc:methodik:versuchsaufbau:daten:qc1} zeigt auszugsweise ein Beispiel für die Rückgabe der Daten für John Doe aus Tabelle \ref{tbl:empfehlungssysteme:arbeitsweise:tbl1}.
...
\lstinputlisting[
	language=json,
	caption=Beispiel für einen Auszug aus einem Mitarbeiter-\acsu{JSON} der \acsu{REST}-Schnittstelle des Intranets der EXXETA AG,
	captionpos=b,
	label=qc:methodik:versuchsaufbau:daten:qc1
]{gfx/john.json}
\end{minipage}

Vor Berechnung der Katz-Zentralität werden die Bewertungen der Fähigkeiten der Mitarbeiter um eins erhöht. Damit symbolisiert ein Kantengewicht von null ausschließlich nicht vergebene Beurteilungen. Außerdem wird auf eine Mittelwert-Zentrierung verzichtet. 
