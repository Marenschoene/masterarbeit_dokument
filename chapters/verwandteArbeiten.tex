...
Die aus den drei Ansätzen (a), (b) und (c) erhaltenen Vertrauenswerte zwischen bestehenden Teammitgliedern und potentiellen Kandidaten aggregierten \textcite[S. 7ff.]{malinowski:2005} unter Berücksichtigung der Anzahl gemeinsam bewerteter Stellenprofile zu einer finalen Vertrauensbewertung auf.
...
In einer weiteren Veröffentlichung stellte \textcite{keim:2007} aufbauend auf den Erkenntnissen von \textcite{faerber:2003}, \textcite{keim:2005}, \textcite{malinowski:2005} und \textcite{malinowski:2006} das in Abbildung \ref{fig:verwandteArbeiten:abb3} dargestellte Empfehlungssystem vor. Diese verfolgte das Ziel, passende Arbeitspartner zu finden und dabei sowohl Fähigkeiten als auch zwischenmenschliche Attribute zu berücksichtigen.
...
In Abbildung \ref{fig:verwandteArbeiten:abb3} ist zu erkennen, dass das Empfehlungssystem von \textcite{keim:2007} aus vier Komponenten besteht. Auf der linken Seite sind dabei zwei Module zur Stellen- bzw. Kandidatensuche und auf der rechten Seite zwei Komponenten zur Teambesetzung dargestellt. Das Lebenslaufempfehlungssystem (CV recommender) entspricht der von \textcite{faerber:2003} entwickelten Anwendung, welche anhand von Abbildung \ref{fig:verwandteArbeiten:abb1} vorgestellt wurde. Beim Stellenempfehlungssystem (Job recommender) handelt es sich um die von \textcite{malinowski:2006} adaptierte Version des Lebenslauf-Empfehlungssystems mit welcher Personen zu ihren Präferenzen passende Stellenausschreibungen ermitteln konnten. Das vorhersagende Vertrauensmodul (Predictive Trust module) entspricht dem anhand von Abbildung \ref{fig:verwandteArbeiten:abb2} behandelten Ansatz zur vorhersage von Vertrauensbeziehungen zwischen potentiellen Teammitgliedern. Dieses wurde in den Publikationen von \textcite{keim:2005} und \textcite{malinowski:2005} detailliert vorgestellt. Vergleichbar zur Veröffentlichung von \textcite{keim:2005} ist auch das beschreibende Vertrauensmodul (Descriptive trust module) vor. Dieses erfasst explizit vergebene Vertrauensbewertungen von Teammitgliedern in Form einer Ontologie und repräsentiert diese in Form eines Graphen. Über diese Darstellung können Anwender soziale Beziehungen durchsuchen und analysieren. Auf diese Weise sollen sie besser entscheiden können, mit welchen Personen sie eine Zusammenarbeit eingehen möchten.

Ähnlich zur Publikation von \textcite{malinowski:2006} muss auch zur Veröffentlichung von \textcite{keim:2007} kritisch bemerkt werden, dass die Forscher die Resultate der einzelnen Module nicht zu einem Ergebnis zusammenfassten. Die Implementierung der oben in Abbildung \ref{fig:verwandteArbeiten:abb3} dargestellten Aggregationsebene (Aggregation layer) ließen die Wissenschaftler offen. Das von \textcite{keim:2007} entwickelte System enthält mit dem beschreibenden und dem vorhersagenden Vertrauensmodul damit zwar Komponenten, welche bilaterale Vertrauensbeziehungen modellieren bzw. bei der Empfehlungsbestimmung berücksichtigen. Somit handelt es sich bei der Suche nach potentiell vertrauenswürdigen Teamkollegen um einen bilateralen Prozess, da Präferenzen von bestehenden Teammitgliedern und potentiellen Kandidaten berücksichtigt werden. Hierbei wird jedoch nicht beachtet, ob die Person auch fachlich für die Stelle geeignet ist oder ob die Position inhaltlich den Vorstellungen des Kandidaten entspricht.
