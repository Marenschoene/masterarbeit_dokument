...
Um dieses Ziel zu erreichen, sah es das System von \textcite{malinowski:2005} bei der Besetzung einer Stelle in einem bestehenden Projektteam zunächst vor, aus allen verfügbaren Personen über das Verfahren von \textcite{faerber:2003} die N fachlich geeignetsten Kandidaten zu bestimmen. Diese dienten als Eingabe für eine weitere Komponente, über welche die Wissenschaftler das potentielle Vertrauen zwischen möglichen Kandidaten und vorhanden Teammitgliedern berechneten. Um das Vertrauen zu bestimmen, nutzten die Forscher drei separate Ansätze. Diese sind in Abbildung \ref{fig:verwandteArbeiten:abb2} dargestellt.
...
Die erhaltenen Vertrauenswerte zwischen den Personen aggregierten \textcite{malinowski:2005} unter Berücksichtigung der Anzahl gemeinsam bewerteter Stellenprofile zu einer finalen Vertrauensbewertung.

Im letzten Schritt des Systems errechneten \textcite{malinowski:2005} aus den Ergebnissen der Komponente von \textcite{faerber:2003} und ihres Vertrauensmoduls eine gemeinsame Ergebnisliste. Hierzu wendeten sie die folgende Formel \ref{frml:verwandteArbeiten:formel2} an:
\begin{equation}
	R' = \alpha * t'_{M*y} * (1-\alpha) * r'_{x,y,v}
	\label{frml:verwandteArbeiten:formel2}
\end{equation}
In Formel \ref{frml:verwandteArbeiten:formel2} stand $t'_{M*y}$ für die Liste der Vertrauensbeziehungen und $r'_{x,y,v}$ für die fachlichen Bewertungen der Kandidaten. Für $\alpha$ setzten die Wissenschaftler den Wert 0.5 ein. Jedoch merkten \textcite{malinowski:2005} zu Formel \ref{frml:verwandteArbeiten:formel2} an, dass diese noch nicht optimal sei und weiterer Forschung bedarf.
% Evaluation dieses Systems fehlt in Paper
