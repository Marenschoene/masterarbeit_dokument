...
\begin{table}[h]
	...
	\begin{tabular}{c|c|c|c|c|c|c|c}
	 & Python & Java & MySQL & MongoDB & HDFS & Spark & ...\\ 
	\hline
	Doe, Jane & ? & 4 & 3 & 3 & 1 & ? & ...\\
	Doe, John & 3 & ? & 2 & ? & ? & ? & ...\\
	Muster, Erika & ? & 5 & ? & ? & 5 & 3 & ...\\
...
Zur Ähnlichkeitsberehnung können unterschiedliche Algorithmen wie die Jaccard-Ähnlichkeit \cite[S. 2]{bharti:2019}, die euklidische Distanz \cite[S. 3]{cheng:2013} oder die Kosinus-Ähnlichkeit \cite[S. 2]{duong:2018}. Beispielhaft ist in der folgenden Gleichung \ref{fig:empfehlungssysteme:cf:speicherbasiert:formel1} die Formel zur Berechnung der Kosinus-Ähnlichkeit zwischen den Objekten A und B dargestellt \cite[S. 111]{bharti:2019}:
\begin{equation}
...
\end{equation}
Wie in Gleichung \ref{fig:empfehlungssysteme:cf:speicherbasiert:formel3} dargestellt, kann auch die elementbasierte Ähnlichkeit zwischen Python und MySQL aus Tabelle \ref{tbl:empfehlungssysteme:arbeitsweise:tbl1} mittels Kosinus-Distanz berechnet werden:
\begin{equation}
...
Das Sparsity Problem kann auch in Tabelle \ref{tbl:empfehlungssysteme:arbeitsweise:tbl1} beobachtet werden. Dort haben MongoDB und Spark jeweils nur eine Bewertung. Für diese Fähigkeiten ist es folglich über Ähnlichkeitsberechnungen im Bereich des speicherbasierten kollaborativen Filterns nicht möglich, robuste Vorhersagen zu ermitteln. Aus diesem Grund überführten \textcite[S. 2f.]{huang:2004} die bisher verwendete Matrixdarstellung in die Form eines bipartiten Graphen. Diese Datenstruktur zeichnet sich durch die Verwendung zwei unterschiedlicher Arten von Knoten aus \cite[S. 1f.]{cao:2021}. \textcite{huang:2004} speicherten Nutzer und Elemente in Form zwei verschiedener Arten von Knoten. Transaktionen und Bewertungen stellten die Forscher als gewichtete Kanten zwischen beiden Knotentypen dar. Abbildung \ref{fig:empfehlungssysteme:cf:speicherbasiert:abb2} zeigt die Fähigkeitsmatrix aus Tabelle \ref{tbl:empfehlungssysteme:arbeitsweise:tbl1} in der Datenstruktur eines solchen bipartiten Graphen. In diesem Beispiel sind die Knoten der Mitarbeiter rot und die Knoten der Fähigkeiten blau markiert.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[node distance={35mm}, thick, main/.style = {draw, circle}] 
		...
		\node[main, fill=itemcolor] (Java) [below right of=MongoDB] {$Java$}; 
		\node[main, fill=itemcolor] (MySQL) [above right of=Java] {$MySQL$}; 
		\node[main, fill=itemcolor] (Python) [below right of=MySQL] {$Python$}; 
		\node[main, fill=itemcolor] (HDFS) [above right of=Python] {$HDFS$}; 
		...
		\node[main, fill=usercolor] (Erika) [below of=Max] {$Erika M.$}; 
		
		\draw (Jane) -- node[midway, right] {4} (Java);
		\draw (Jane) -- node[midway, above] {3} (MySQL);
		\draw (Jane) -- node[midway, above] {3} (MongoDB);
		\path (Jane) edge[bend left=90] node[midway, above] {1} (HDFS);
		
		\draw (John) -- node[midway, right] {3} (Python);
		\draw (John) -- node[midway, above] {2} (MySQL);
		
		\draw (Erika) -- node[midway, above] {5} (Java);
		\path (Erika) edge[bend right=10] node[midway, right] {5} (HDFS); 
		\draw (Erika) -- node[midway, above] {3} (Spark);
		
		\draw (Max) -- node[midway, above] {2} (Python);
		\draw (Max) -- node[midway, above] {3} (Java);
