...
Ein häufig angewendetes Verfahren ist die Katz-Zentralität. Diese wird im Kontext graphenbasierter Empfehlungssysteme oft zur Verbindungsvorhersage (Link Prediction) genutzt. Diese Problemstellung ist unter anderem im Bereich der sozialen Medien verbreitet und verfolgt das Ziel, aus vorhandenen Kanten im Graphen bisher unbekannte Verbindungen vorherzusagen. \textcite{huang:2004} empfehlen zur Berechnung der Katz-Zentralität, zunächst zwei Werte für die Variablen $M$ und $\beta$ festzulegen. $M$ steht dabei für die maximal zulässige Anzahl an Kanten, die zwischen den beiden Knoten liegen dürfen, für welche eine Verbindung vorhergesagt werden soll. $\beta$ steht für eine Zahl im Bereich von null bis eins. Diese kann frei festgelegt werden. \textcite{huang:2004} empfehlen, den Wert umso weiter an eins anzunähern, je stärker transitive Beziehungen in der Berechnung gewichtet werden sollen. Die Katz-Zentralität kann anschließend über die folgende Gleichung \ref{frml:empfehlungssysteme:cf:formel1} bestimmt werden:
\begin{equation}
	W(x,y) := \sum_{l=1}^{M} \beta^{l} * |paths_{x,y}^{\langle l \rangle}|
	\label{frml:empfehlungssysteme:cf:formel1}
\end{equation}
Zur Anwendung von Gleichung \ref{frml:empfehlungssysteme:cf:formel1} wird $l$ zunächst auf eins gesetzt und bis der Wert $M$ erreicht ist, in jedem Durchgang um zwei erhöht. In jedem Durchlauf werden alle unabhängigen Wege zwischen zwei den beiden Zielknoten bestimmt, welche über $l$ Kanten zu erreichen sind und in $|paths_{x,y}^{\langle l \rangle}|$ eingesetzt. Die Kantengewichte werden dabei als mehrere parallele Kanten gewertet. Die Multiplikation mit $\beta^{l}$ hat zur Folge, dass längere Wege eine schwächere Auswirkung auf das Endergebnis haben.

Würde anhand von Gleichung \ref{frml:empfehlungssysteme:cf:formel1} beispielsweise die Katz-Zentralität für Jane Doe und Java mit $M=3$ und $\beta=0.5$ bestimmt werden, ergäbe sich als Ergebnis der Wert $0.625$. Dieser entsteht, da der Knoten Java von Jane Doe über fünf unabhängigen Wege erreicht werden kann. Diese lauten: $2*$(Jane D., Python, Max M., Java), $1*$(Jane D., MySQL, Max M., Java) und $2*$(Jane D., MySQL, John D., Java). Dabei entsteht die folgende Rechnung \ref{frml:empfehlungssysteme:cf:formel2}:
\begin{equation}
	W(Jane D., Java) := 0.5^{1}*0+0.5^{3}*(2+1+2) = 0.625
	\label{frml:empfehlungssysteme:cf:formel2}
\end{equation}

\textcite{guns:2014, libenNowell:2007}

$\beta$ darf nicht größer sein als $1/\lambda_1$ --> Größter Eigenvektor
