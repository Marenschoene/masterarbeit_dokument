...
Um speicherbasierte Verfahren umzusetzen, werden nachbarschaftsbasierte Algorithmen eingesetzt. Soll hierbei beispielsweise die fehlende Java-Bewertung von Jane Doe in Tabelle \ref{tbl:empfehlungssysteme:arbeitsweise:tbl1} vorhergesagt werden, erfolgt dies über paarweise Ähnlichkeitsberechnungen \cite[S. 2f.]{bharti:2019}. Verwenden Algorithmen bei der Empfehlungsbestimmung die Ähnlichkeiten zwischen Mitarbeitern bzw. Tabellenzeilen zur Vorhersage, werden diese als nutzer-orientiert (user-oriented) bezeichnet. Greifen Verfahren dagegen auf die Gleichartigkeit zwischen Fähigkeiten bzw. Tabellenspalten zurück, werden sie element-orientiert (item-oriented) genannt \cite[S. 1f.]{duong:2018}. Laut \textcite[S. 42]{recommenderSystems:2016} liefern element-orientierte Verfahren häufig präzisere und nutzer-orientierte Methoden diversere Ergebnisse. Somit können die Vorschläge der erstgenannten Ansätze für den Nutzer relevanter sein. Jedoch ist die Wahrscheinlichkeit hoch, dass diesem auch keine andere Empfehlung zusagt, wenn ihm ein Vorschlag missfällt. 

Bei der Implementierung beider Verfahren kommen häufig \ac{KNN}-Algorithmen zum Einsatz \cite[S. 1]{nayak:2018}. Um diese anzuwenden wird bei element-orientierten Verfahren die Ähnlichkeit zwischen Java und jeder anderen Fähigkeit bestimmt, welche Jane Doe beherrscht. Anschließend wird die durchschnittliche Bewertung der k-ähnlichsten Elemente berechnet und als Java-Beurteilung für Jane Doe eingesetzt \cite[S. 2]{hao:2013}.
...
\end{equation}
Anstelle der Vektoren A und B können in Gleichung \ref{fig:empfehlungssysteme:cf:speicherbasiert:formel1} im Beispiel aus Tabelle \ref{tbl:empfehlungssysteme:arbeitsweise:tbl1} paarweise Spalten/Fähigkeiten bzw. Zeilen/Mitarbeiter eingesetzt werden.
...
\end{equation}
Wie in der folgenden Gleichung \ref{fig:empfehlungssysteme:cf:speicherbasiert:formel3} dargestellt, kann auch die elementbasierte Ähnlichkeit beispielsweise für Java und MySQL aus Tabelle \ref{tbl:empfehlungssysteme:arbeitsweise:tbl1} mittels Kosinus-Distanz berechnet werden:
\begin{equation}
...
\textcite[S. 7ff.]{huang:2004} bemerken mit Blick auf biparithe Graphen, dass bei klassischen nachbarschaftsbasierten Algorithmen nur Empfehlungen für Knoten bestimmt werden können, welche von einem Zielknoten über drei Kanten zu erreichen sind. Somit könnten in der vorliegenden Abbildung \ref{fig:empfehlungssysteme:cf:speicherbasiert:abb2} über solche Verfahren keine Bewertungs-Vorhersagen für Jane Doe und Spark bzw. Erika Muster und MongoDB bestimmt werden. Um auch in solchen Fällen Empfehlungen generieren zu können, ist der Einsatz graphenbasierter Algorithmen empfehlenswert. Diese beziehen über transitive Verbindungen auch weiter entfernte Knoten in die Berechnung mit ein \cite[S. 60f.]{recommenderSystems:2016}.
...
Modellbasierte Ansätze verwenden Verfahren aus den Bereichen des Data Minings und des maschinellen Lernens zur Generierung von Vorschlägen. Hierbei berechnen Wissenschaftler statistische Modelle, bevor sie ihr Empfehlungssystem ihren Nutzern zur Verfügung stellen. Dieses Vorgehen hat den Vorteil, dass in der Produktivumgebung eingesetzte Recommender Eninges auch bei großen Datenmengen wesentlich effizienter Vorschläge bestimmen können als es bei speicherbasierten Verfahren möglich ist \cite[S. 8]{yang:2016}.

In der Literatur sind verschiedenste Verfahren zur Implementierung von modellbasierten Empfehlungssystemen wie Bestimmung neuronaler Netze \cite[S. 5ff.]{personJobFit:2018}, Anwendung des naiven Bayes-Klassifikators \cite[S. 2]{valdividezodiaz:2019}, Durchführung von Matrix-Faktorisierungsverfahren \cite[S. 2ff.]{ortega:2016} und Berechnung von Entscheidungsbäumen \cite[S. 1ff.]{yu:2012} verbreitet. 

Exemplarisch wird im folgenden Teil das Lernen eines Modells auf Basis eines Entscheidungsbaumes demonstriert. Zur Vereinfachung der Berechnung zeigt Tabelle \ref{tbl:empfehlungssysteme:cf:modellbasiert:tbl1} die Bewertungen von nur zwei Fähigkeiten mit boolschen Werten.

\newcolumntype{g}{>{\columncolor{itemcolor}}c}
\begin{table}[h]
	\centering
	\begin{tabular}{c|c|c|g}
		& C & C\# & C++\\ 
		\hline
		Doe, Jane     & false & true  & false\\
		Doe, John     & true  & false & true\\
		Muster, Erika & false & false & true\\
		Muster, Max   & true  & false & true\\
		... 		  & ...   & ...   & ...\\ 
	\end{tabular}
	\caption{ Matrixdarstellung von Fähigkeiten mit boolschen Bewertungen}
	\label{tbl:empfehlungssysteme:cf:modellbasiert:tbl1}
\end{table}

In Tabelle \ref{tbl:empfehlungssysteme:cf:modellbasiert:tbl1} ist die letzte Spalte blau eingefärbt. Diese enthält in diesem Beispiel die abhängige Variable, deren Wert über die übrigen unabhängigen Variablen in den restlichen Tabellenspalten vorhergesagt werden soll.

\newpage
% Es könnte noch ein Satz rein, wann sich eher itembased und wann userbased eignet
- \cite[S. 2]{weightedSimilarity:2015}: Deutlich weniger Rechenzeit
- \cite[S. 8]{yang:2016}: Modellerstellung offline
- Su  and  Khoshgoftaar: Vorteil speicherbasiert: Neue Nutzer können einfach hinzugefügt werden --> Geht bei modelbased nicht
- Speicherbasiert ist laut Su einfacher zu implementieren
% Fluch der Dimensionalität --> https://reader.elsevier.com/reader/sd/pii/S1877050915007462?token=004D1714622D9F8D4ABCC3AEFB3824FDA9C5E08FFA6365E5EF57C7B6B3F51F5B68F07DAE7435E7A7B85D5506C1EA4CE7&originRegion=eu-west-1&originCreation=20210527150831 --> Bei PCR gehen Informationen verloren
% 3. Cold Start

Ein Problem, welches weder speicher- noch modellbasierte Verfahren beseitigen können, ist das Phänomen des Cold Starts.

Ob eine speicher- oder modellbasierte Implementierung geeigneter ist, müssen HR-Manager im Einzelfall je nach Situation in ihrem Unternehmen bewerten. \textcite[S. 4]{peerToPeer:2008} stellen fest, dass speicherbasierte Ansätze in der Praxis häufig wesentlich einfacher umzusetzen sind, da weniger Parameter aufeinander abgestimmt werden müssen. Modellbasierte Ansätze benötigen dafür im produktiven Einsatz eine kürzere Berechnungszeit, welche laut \textcite[S. 2]{weightedSimilarity:2015} insbesondere bei steigender Datenmenge vorteilhaft ist.\\
Um die Vorzüge verschiedener Verfahren zu vereinen, ist es außerdem möglich, mehrere der vorgesteellten Ansätze in einer Software zu kombinieren. Solche Anwendungen werden als hybride Empfehlungssysteme bezeichnet \cite[S. 8]{malinowski:2008}. Diese eigenen sich auch für einen guten Umgang mit dem Cold Start- und dem Sparse Data-Problem.
%bieten modellbasierte Verfahren einen einfacheren Umgang mit dem Cold Start- und dem Sparse Data-Problem \cite[S. 4]{peerToPeer:2008}.
