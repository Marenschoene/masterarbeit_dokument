...
\end{equation}
In Gleichung \ref{frml:empfehlungssysteme:cf:speicherbasiert:formel4} steht $M$ für die Adjazenzmatrix des betrachteten Graphen \cite[S. 4]{libenNowell:2007}. Diese gibt an, wie viele Kanten zwischen den jeweiligen Knoten bestehen \cite[S. 6]{guns:2014}. Tabelle \ref{tbl:empfehlungssysteme:arbeitsweise:tbl2} zeigt für den Graphen aus Abbildung \ref{fig:empfehlungssysteme:cf:speicherbasiert:abb2}, wie viele Verbindungen von einem Knoten (Zeile) im  zu jedem anderen Knoten (Spalte) existieren.
...
Der Parameter $\beta$ steht in Gleichung \ref{frml:empfehlungssysteme:cf:speicherbasiert:formel4} für eine Zahl im Bereich von null bis eins \cite[S. 6]{guns:2014}. Diese muss jedoch stets kleiner als $\frac{1}{\lambda}$ sein. $\lambda$ entspricht dabei dem größten Eigenwert der untersuchten Matrix \cite[S. 6]{zhan:2017}. Je größer der Wert von $\beta$ angesetzt wird, desto stärker werden weit entfernte Beziehungen in der Berechnung gewichtet \cite[S. 6]{guns:2014}.
...
\textcite{zarei:2019, shang:2008, guns:2014}
