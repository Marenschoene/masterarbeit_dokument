...
\subsection{Zuordnung von Personen in verwandten Domänen}
...
Im Rahmen eines Experiments evaluierten die Autoren das entwickelte System.
...
% Im umkehrschluss bedeutet das, es war den Autoren nicht möglich zu ermitteln, ob bspw. eine Gewichtung von 0.7 für wc insgesamt zu mehr positiven interaktionen im system führen würde, da nicht bemessen werden konnte, wie ein Empfehlungsempfänger auf eine empfohlene Person reagiert.
% So konnte nicht bemessen werden, wie sich die Interaktionen verhalten, wenn bspw. ein Empfehlungsempfänger eine empfehlung annimmt, obwohl die eigentlich nicht seinen Interessen entspricht.
% D.h. die hätten krassere Schwankungen erwartet, wenn die Interaktionen über einen gewissen zeitraum betrachtet worden wären, und nicht in der Vergangenheit, d.h. dass die positiven Interaktionen ingesamt steigen würden.
...
% Damit beschreiben die Autoren die Art, welche Kennzahl erhoben wird.
% Da die Interaktionen des sozialen Netzwerks basierend auf dem Interesse des
% Damit beschreiben sie den Zustand, dass das Senden einer Nachricht und das Empfangen derselbigen in dem sozialen Netzwerk zeitlich auseinander liegen.
% Demnach kann das Gewicht einer empfohlenen Person für
% Dieser Zustand ist auf die Art, wie die Kennzahl ermittelt wird.
...
\textcite[S. 131ff.]{kleinerman:2:inproceedings} stellen fest, dass in Forschungsarbeiten bislang offen blieb, wie die unilateralen Präferenzen zweier Parteien optimal im Gleichgewicht gehalten werden können.
...
Um für Nutzer ohne erfolgreiche Interaktionen in der Vergangenheit Empfehlungen zu erstellen, definieren die Autoren zusätzlich ein Optimierungsproblem über alle Nutzer eines Systems hinweg \cite[S. 135]{kleinerman:2:inproceedings}:\footnote{Die Formel von \textcite[S. 135]{kleinerman:2:inproceedings} enthält einen Tippfehler ($\geq$ anstelle von $\leq$ [sic]).}
...
Insgesamt zeigen die Ergebnisse der Evaluation von \textcite[S. 62]{link:booklet}, dass die Zufriedenheit der Mitarbeiter sowie deren erwartete Arbeitsleistung seitens der Manager durch den bilateralen Ansatz in vier von fünf Fällen gesteigert werden konnte.
...
Zuletzt veröffentlichten \textcite[S. 102ff.]{yang:inproceedings} einen Ansatz für die Bestimmung des Person-Job-Fits unter Berücksichtigung wechselseitiger Präferenzen.
...
% Die Darstellung der Interaktionen als dual-perspective Graph sind in Abbildung \ref{fig:relatedwork:abb3} dargestellt.
% \begin{figure}[H]
%     \centering
% 	\includegraphics[width=0.9\textwidth]{gfx/dpg.png}
% 	\caption[Dual-perspective Graph]{Dual-perspective Graph\\
%     (Eigene Darstellung in Anlehnung an \cite[S. 104]{yang:inproceedings})}
% 	\label{fig:relatedwork:abb3}
% \end{figure}
...
% Kandidaten bzw. Jobausschreibungen werden jeweils durch eine Beschreibung (z.B. Anschreiben, Jobausschreibung) sowie gerichtete Interaktionen (z.B. Bewerbung, Sichten von Bewerbungsunterlagen) repräsentiert werden \cite[S. 103]{yang:inproceedings}.
...
Die Autoren vergleichen ihren Ansatz bezüglich der Performance (Precision, Recall, Normalized Discounte Cumulative Gain und Mean Reciprocal Rank) mit diversen Baseline-Algorithmen.
...
\textcite[S. 15f.]{rodriguez:inproceedings} prüften ihren Ansatz anhand von 760 offenen Jobangeboten, von denen jedes zwischen 6 und 9000 mal in Empfehlungen angezeigt wurde.
