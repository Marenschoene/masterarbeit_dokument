...
Neben dem Aggregation-Function Ansatz nennen \textcite[S. 861]{adomavicius:4:inbook} probabilistische Modellierungs-Ansätze (engl.: Probabilistic Modeling Approach) für die Integration multi-kriterieller Bewertungen in Empfehlungssystemen.
...
\begin{figure}
    \centering
    \subfloat[Flexible Mixture Model]{\includegraphics[height=0.9in]{gfx/f-m-m.png}\label{fig:optimierung:loesungen:abb2:1}}\\
    \subfloat[Flexible Mixture Model mit multi-kriteriellen Bewertungen und Dependency-Struktur]{\includegraphics[height=2.20in]{gfx/f-m-m-2.png}\label{fig:optimierung:loesungen:abb2:2}}\\
...
% Im Detail stellen die Autoren zwei multi-kriterielle Herangehensweisen vor, welche auf der \ac{PLSA} aufbauen \cite[S. 864]{adomavicius:4:inbook}.
% Die \ac{PLSA} \cite[S. 89ff.]{hofmann:article} ist ein statistisches Konzept, das ebenfalls ein Vorhandensein latenter Variablen unterstellt.
% Im Vergleich zum \ac{FMM} existiert in der \ac{PLSA} nur ein Set $Z$ an latenten Variablen \cite[S. 2]{si:inproceedings}.
...
% HIER WEITERMACHEN, checken wie ich Zugriff auf das Paper bekomme: https://dl.acm.org/doi/10.5555/1574514.1574517

\textbf{Multilinear Singular Value Decomposition Ansatz}
...
Nach \textcite[S. 94]{recommenderSystems:2016} ist Matrix Factorization die Repräsentation einer $m \times n$-Matrix $R$ als Produkt einer $m \times u$-Matrix $U$ und einer $n \times v$-Matrix $V$ \cite[S. 94]{recommenderSystems:2016}.
...
Gemäß \textcite[S. 864]{adomavicius:4:inbook} wird \ac{SVD} maßgeblich für das Auffinden von Faktor-Räumen geringerer Dimensionalität angewandt.
Bewertungen in Empfehlungssystemen können mithilfe der vollständigen latenten Vektoren ermittelt werden.
...
% Hier WEITERMACHEN!
In Anlehnung an die Darstellung dreidimensionaler Daten in multidimensionalen Systemen\footnote{Vgl. Kapitel \ref{ch:empfehlungssysteme:preferences:data}}, werden multi-kriteriellen Bewertungen in dem \ac{MSVD} Ansatz in einem $Nutzer \times Element \times Kriterien$-Tensor abgebildet \cite[S. 1235]{li:2:inproceedings}.
% Hier Erklären, wie von SVD zu MSVD kommt (bild zeigen?)
% Multilineare svd bedeutet ... siehe letzten beiden Paper: 

% Die Matrix Factorization ist am Beispiel der \ac{SVD} in Abbildung \ref{fig:optimierung:loesungen:abb3} grafisch dargestellt.

% \begin{figure}[H]
%     \centering
% 	\includegraphics[width=1.0\textwidth]{gfx/svd.png}
% 	\caption[Singular Value Decomposition]{Singular Value Decomposition\\
%     (Eigene Darstellung in Anlehnung an \cite[S. 1260]{lathauwer:article})}
